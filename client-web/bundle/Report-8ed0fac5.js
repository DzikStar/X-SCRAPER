"use strict";
(self.webpackChunk_twitter_responsive_web = self.webpackChunk_twitter_responsive_web || []).push([
    ["bundle.Report-8ed0fac5"],
    {
        545334: (e, t, s) => {
            s.d(t, { d: () => a });
            var n = s(111677);
            const a = s.n(n)().gf5e9ea6;
        },
        847607: (e, t, s) => {
            s.d(t, { Aq: () => u, FK: () => S, GS: () => i, N1: () => E, PY: () => l, X6: () => m, cm: () => d });
            s(202784);
            var n = s(111677),
                a = s.n(n),
                r = s(391366),
                o = s(40904);
            const i = a().hb568af4,
                d = a().bb1cbeb6,
                c = (a().h2f62206, a().e67b2d65),
                _ = a().f05597b3,
                u = (e) => c({ screenName: e }),
                l = (e) => _({ screenName: e }),
                E = (e, t, s) => {
                    const { muting: n, screen_name: a } = e;
                    return { text: n ? l(a) : u(a), onClick: n ? s : t, Icon: n ? r.default : o.default };
                },
                I = a().h59f52ee,
                p = a().eea0cbee,
                v = a().i29533b3,
                A = a().h129c3c3,
                S = (e) => (e ? ((e) => v({ screenName: e }))(e) : I),
                m = (e) => (e ? ((e) => A({ screenName: e }))(e) : p);
        },
        712816: (e, t, s) => {
            s.d(t, { d: () => a });
            var n = s(111677);
            const a = { defaultToast: { text: s.n(n)().b6878b0a }, showToast: !0 };
        },
        879596: (e, t, s) => {
            s.d(t, { D: () => a });
            var n = s(111677);
            const a = { defaultToast: { text: s.n(n)().ca96fe6e }, showToast: !0 };
        },
        234296: (e, t, s) => {
            s.d(t, { Z: () => o, l: () => r });
            var n = s(147143),
                a = s(856674);
            const r = (e, t) => {
                    const s = e.data?.labels?.find((e) => e.label_type === n.w1.PINNED)?.timestamp,
                        a = t.data?.labels?.find((e) => e.label_type === n.w1.PINNED)?.timestamp;
                    return s ? (a ? s - a : -1) : 1;
                },
                o = ({ sort_event_id: e }, { sort_event_id: t }) => (e ? (t ? (0, a.ZP)(t, e) : 1) : -1);
        },
        198506: (e, t, s) => {
            s.d(t, { Z: () => c, R: () => _ });
            var n = s(662678),
                a = s(275365),
                r = s(856674);
            const o = (e = 0) => new Date(Number(e)),
                i = (e, t) => o(e.join_time) - o(t.join_time),
                d = ({ user: e }, { user: t }) => (0, r.ZP)(e.id_str, t.id_str),
                c = (e, t) => {
                    const { created_by_user_id: s, participants: r, type: o } = e,
                        [c, _] = (0, n.Z)(r, ({ user: e }) => e.id_str === t);
                    return _.length
                        ? o === a.eD.GROUP && 1 === _.length
                            ? [...c, ..._]
                            : s
                              ? _.sort(
                                    (
                                        (...e) =>
                                        (t, s) =>
                                            e.map((e) => e(t, s)).filter((e) => !!e)[0] || 0
                                    )(
                                        (
                                            (e) =>
                                            ({ user: t }, { user: s }) => {
                                                switch (e) {
                                                    case t.id_str:
                                                        return -1;
                                                    case s.id_str:
                                                        return 1;
                                                    default:
                                                        return 0;
                                                }
                                            }
                                        )(s),
                                        i,
                                        d,
                                    ),
                                )
                              : _
                        : c;
                },
                _ = (e, t) => (e.includes("-") ? e.replace(t, "").replace(/[e-]/g, "") : null);
        },
        730996: (e, t, s) => {
            s.d(t, { W: () => a, g: () => n });
            s(571372);
            const n = Object.freeze({ AppealTweetWarning: "appealtweet", LimitedDiscoveryAppealTweet: "limiteddiscoveryappealtweet", ProfileOnlyDiscoveryAppealTweet: "profileonlydiscoveryappealtweet", DMConversation: "reportdmconversation", DMMessage: "reportdmconversation", HideCommunityTweet: "hidetweet", Moment: "reportmoment", RemoveCommunityMember: "removecommunitymember", Tweet: "reporttweet", List: "reportlist", User: "reportprofile", Space: "reportspace", AppealSuspension: "appealsuspension", Notification: "notification" });
            function a(e) {
                switch (e) {
                    case "appealtweet":
                    case "limiteddiscoveryappealtweet":
                    case "profileonlydiscoveryappealtweet":
                    case "reportdmconversation":
                    case "hidetweet":
                    case "reportmoment":
                    case "removecommunitymember":
                    case "reporttweet":
                    case "reportlist":
                    case "reportprofile":
                    case "reportspace":
                    case "appealsuspension":
                    case "notification":
                        return e;
                    default:
                        throw new Error(`unexpected source ${e}`);
                }
            }
        },
        275365: (e, t, s) => {
            s.d(t, { BU: () => c, Cr: () => r, To: () => a, UN: () => d, eD: () => n, td: () => o, v1: () => i });
            const n = Object.freeze({ ONE_TO_ONE: "ONE_TO_ONE", GROUP: "GROUP_DM", SECRET_ONE_TO_ONE: "SECRET_ONE_TO_ONE" }),
                a = Object.freeze({ AT_END: "AT_END", HAS_MORE: "HAS_MORE" }),
                r = Object.freeze({ CONVERSATION_AVATAR_UPDATE: "conversation_avatar_update", CONVERSATION_NAME_UPDATE: "conversation_name_update", CONVERSATION_PROFILE_INFO_HEADER: "conversation_profile_info_header", CONVERSATION_READ: "conversation_read", CONVO_METADATA_UPDATE: "convo_metadata_update", DELEGATE_ALERT_BANNER: "delegate_alert_banner", DISABLE_NOTIFICATIONS: "disable_notifications", ENABLE_NOTIFICATIONS: "enable_notifications", END_AV_BROADCAST: "end_av_broadcast", ENCRYPTED_CONVERSATION: "encrypted_conversation", JOIN_CONVERSATION: "join_conversation", LOADING_INDICATOR: "loading_indicator", MARK_ALL_AS_READ: "mark_all_as_read", MENTION_NOTIFICATIONS_UPDATE: "mention_notifications_setting_update", MESSAGE: "message", MESSAGE_DELETE: "message_delete", MESSAGE_MARK_AS_NOT_SPAM: "message_unmark_as_spam", MESSAGE_MARK_AS_SPAM: "message_mark_as_spam", NEW_MESSAGES_DIVIDER: "new_messages_divider", PARTICIPANTS_JOIN: "participants_join", PARTICIPANTS_LEAVE: "participants_leave", REACTION_CREATE: "reaction_create", REACTION_DELETE: "reaction_delete", READ_ONLY_INDICATOR: "read_only_indicator", REMOVE_CONVERSATION: "remove_conversation", TRUST_CONVERSATION: "trust_conversation", TYPING_INDICATOR: "typing_indicator", WELCOME_MESSAGE: "welcome_message_create" }),
                o = Object.freeze({ MISSED: "MISSED", CANCELED: "CANCELED", DECLINED: "DECLINED", HUNG_UP: "HUNG_UP", TIMED_OUT: "TIMED_OUT" }),
                i = Object.freeze({ AUDIO_ONLY: "AUDIO_ONLY", VIDEO: "VIDEO" }),
                d = Object.freeze({ MUTUAL_FRIENDS: "mutual_friends" }),
                c = Object.freeze({ UNINITIATED: "UNINITIATED", EXISTING: "EXISTING", DEVICE_NOT_A_MEMBER: "DEVICE_NOT_A_MEMBER" });
        },
        182991: (e, t, s) => {
            s.d(t, { x: () => a });
            var n = s(147143);
            const a = (e, t) => (e[n.Yf] ? e[n.Yf].conversations[t] : null);
        },
        613195: (e, t, s) => {
            s.d(t, { $S: () => Ue, Ae: () => Pe, Ap: () => xe, BW: () => Ze, Be: () => pe, Ek: () => je, FI: () => Ve, FS: () => te, GP: () => Te, Ke: () => Fe, OF: () => oe, OW: () => Ie, PZ: () => Le, Pr: () => X, Qe: () => Xe, S9: () => Ne, SF: () => fe, UA: () => ce, Um: () => ue, W: () => de, X5: () => ze, Xw: () => Qe, ZP: () => W, Zg: () => ne, _D: () => qe, aG: () => ie, ax: () => se, ck: () => _e, d0: () => De, dq: () => we, gQ: () => $, gq: () => ae, jT: () => re, lT: () => Be, nI: () => We, qH: () => ye, qg: () => Ae, sA: () => Ee, sS: () => le, sj: () => me, tF: () => Se, tw: () => Ce, v3: () => ge, wm: () => ve, xu: () => z });
            var n = s(24949),
                a = s(111677),
                r = s.n(a),
                o = s(184605),
                i = s(948322),
                d = s(88656),
                c = s(615656),
                _ = s(856674),
                u = s(917799),
                l = s(312771),
                E = s(56519),
                I = s(497294),
                p = s(390387),
                v = s(877848),
                A = s(601576),
                S = s(919022),
                m = s(275365),
                T = s(627279),
                O = s(720277),
                y = s(849585),
                N = s(147143);
            const R = r().fb4ee2f6,
                C = "rweb/directMessages/conversation/SET_RECIPIENT_CAN_NOT_DM",
                h = "ACTION_DM_ADD_PARTICIPANTS",
                f = "FETCH_DM_CONVERSATION_FUTURE",
                g = "FETCH_DM_CONVERSATION_RELATIVE_HISTORY",
                D = "FETCH_DM_CONVERSATION_HISTORICAL_SNAPSHOT",
                M = u.dg(N.A7, "ACCEPT_CONVERSATION"),
                P = u.dg(N.A7, "MUTE_DM_USER"),
                b = u.dg(N.A7, "ADD_PARTICIPANTS"),
                U = u.dg(N.A7, "FETCH_CONVERSATION"),
                w = u.dg(N.A7, "FETCH_CONVERSATION_FROM_PARTICIPANTS"),
                L = u.dg(N.A7, "LEAVE_CONVERSATION"),
                Z = Object.freeze({ REQUEST: "rweb/directMessages/LEAVE_CONVERSATIONS_REQUEST", SUCCESS: "rweb/directMessages/LEAVE_CONVERSATIONS_SUCCESS", FAILURE: "rweb/directMessages/LEAVE_CONVERSATIONS_FAILURE" }),
                x = u.dg(N.A7, "MARK_CONVERSATION_READ"),
                B = u.dg(N.A7, "DISABLE_MENTION_CONVERSATION_NOTIFICATIONS"),
                V = u.dg(N.A7, "DISABLE_CONVERSATION_NOTIFICATIONS"),
                F = u.dg(N.A7, "ENABLE_MENTION_CONVERSATION_NOTIFICATIONS"),
                Y = u.dg(N.A7, "ENABLE_CONVERSATION_NOTIFICATIONS"),
                j = u.dg(N.A7, "UPDATE_CONVERSATION_NAME"),
                H = u.dg(N.A7, "UPLOAD_MEDIA"),
                q = u.dg(N.A7, "UPDATE_GROUP_AVATAR"),
                k = (e, t) => t + 36e5 * e,
                Q = (e) => [void 0, k(1, e), k(8, e), k(168, e)],
                G = {};
            function W(e = G, t) {
                if (!t) return e;
                switch (t.type) {
                    case M.SUCCESS: {
                        const { conversationId: s } = t.meta,
                            n = e[s];
                        return { ...e, [s]: { ...n, data: { ...n.data, trusted: !0 } } };
                    }
                    case Re: {
                        const s = Object.entries(t.payload || {}).reduce((t, [s, n]) => ((t[s] = { ...K(e[s], n), fetchStatus: { [v.Yj.BOTTOM]: l.ZP.LOADED, [v.Yj.TOP]: l.ZP.LOADED } }), t), {});
                        return { ...e, ...s };
                    }
                    case C: {
                        const s = t.payload;
                        return { ...e, [s]: { recipientCanNotDm: !0 } };
                    }
                    case b.REQUEST: {
                        const s = t.meta.conversationId,
                            n = e[s];
                        return { ...e, [s]: { ...n, fetchStatus: { ...n.fetchStatus, [v.Yj.TOP]: l.ZP.LOADING } } };
                    }
                    case b.FAILURE: {
                        const s = t.meta.conversationId,
                            n = e[s];
                        return { ...e, [s]: { ...n, fetchStatus: { ...n.fetchStatus, [v.Yj.TOP]: l.ZP.FAILED } } };
                    }
                    case b.SUCCESS: {
                        const s = t.meta.conversationId,
                            n = e[s],
                            a = n.data.participants,
                            r = t.payload.entities.entries ? Object.values(t.payload.entities.entries)[0].participants : [];
                        return { ...e, [s]: { ...n, fetchStatus: { ...n.fetchStatus, [v.Yj.TOP]: l.ZP.LOADED }, data: { ...n.data, participants: [...a, ...r] } } };
                    }
                    case U.REQUEST: {
                        const { conversationId: s, direction: n = v.Yj.BOTTOM } = t.meta,
                            a = e[s];
                        return { ...e, [s]: { ...a, fetchStatus: { ...((a && a.fetchStatus) || {}), [n]: l.ZP.LOADING } } };
                    }
                    case U.FAILURE: {
                        const { conversationId: s, direction: n = v.Yj.BOTTOM } = t.meta,
                            a = e[s];
                        return { ...e, [s]: { ...a, fetchStatus: { ...((a && a.fetchStatus) || {}), [n]: l.ZP.FAILED }, error: t.payload } };
                    }
                    case U.SUCCESS: {
                        const { conversationId: s, direction: n = v.Yj.BOTTOM } = t.meta,
                            a = e[s]?.data || {};
                        return { ...e, [s]: { ...e[s], data: { ...a, isSearchResultOnly: !1 }, fetchStatus: { ...((e[s] && e[s].fetchStatus) || {}), [n]: l.ZP.LOADED } } };
                    }
                    case he:
                        return Object.entries(e).reduce((s, [n, a]) => ((s[n] = K(e[n], { last_read_event_id: t.payload })), s), {});
                    case x.REQUEST:
                        return { ...e, [t.meta.conversationId]: K(e[t.meta.conversationId], { last_read_event_id: t.meta.lastReadEventId }) };
                    case P.SUCCESS:
                        return { ...e, [t.meta.conversationId]: K(e[t.meta.conversationId], { muted: t.meta.mute }) };
                    case Me: {
                        const { [t.payload]: s, ...n } = e;
                        return n;
                    }
                    case L.REQUEST:
                        return { ...e, [t.meta.conversationId]: { ...e[t.meta.conversationId], isDeleted: !0 } };
                    case Z.REQUEST: {
                        const s = {};
                        for (const n in t.meta.conversationsIds) Object.assign(s, { id: { ...e[n], isDeleted: !0 } });
                        return { ...e, ...s };
                    }
                    case be:
                        return { ...e, [t.meta.conversationId]: K(e[t.meta.conversationId], { notifications_disabled: !t.payload }) };
                    case V.REQUEST:
                        return { ...e, [t.meta.conversationId]: K(e[t.meta.conversationId], { notifications_disabled: !0, mute_expiration_time: Q(new Date().getTime())[t.meta.duration] }) };
                    case B.REQUEST:
                        return { ...e, [t.meta.conversationId]: K(e[t.meta.conversationId], { mention_notifications_disabled: !0 }) };
                    case Y.REQUEST:
                        return { ...e, [t.meta.conversationId]: K(e[t.meta.conversationId], { notifications_disabled: !1 }) };
                    case F.REQUEST:
                        return { ...e, [t.meta.conversationId]: K(e[t.meta.conversationId], { mention_notifications_disabled: !1 }) };
                    case Ge:
                        return { ...e, [t.meta.conversationId]: K(e[t.meta.conversationId], { avatar_image_https: t.payload }) };
                    case j.REQUEST:
                    case j.SUCCESS:
                        return { ...e, [t.meta.conversationId]: K(e[t.meta.conversationId], { name: t.meta.name }) };
                    case Ye: {
                        const s = e[t.meta.conversationId],
                            n = (s && s.data && s.data.max_entry_id) || "0";
                        return (0, _.T1)(t.payload, n) ? { ...e, [t.meta.conversationId]: K(e[t.meta.conversationId], { max_entry_id: t.payload }) } : e;
                    }
                    case H.REQUEST:
                        return { ...e, [t.meta.conversationId]: K(e[t.meta.conversationId], { isUploading: !0 }) };
                    case H.FAILURE:
                    case H.SUCCESS:
                        return { ...e, [t.meta.conversationId]: K(e[t.meta.conversationId], { isUploading: !1 }) };
                    case He:
                        return { ...e, [t.meta.conversationId]: K(e[t.meta.conversationId], { avatarMediaId: t.payload }) };
                    case ke:
                        return { ...e, [t.meta.conversationId]: K(e[t.meta.conversationId], { avatarMediaId: null }) };
                    case y.t:
                        return G;
                    default:
                        return e;
                }
            }
            const K = (e = {}, t = {}, s = []) => {
                    const { error: n, ...a } = e,
                        { avatarMediaId: r = null } = (0, o.Z)(t.avatarMediaId) ? t : a,
                        { isUploading: i = !1 } = t,
                        { avatarMediaId: d, isUploading: c, max_entry_id: u, min_entry_id: l, ...E } = t,
                        I = { ...(a && a.data), ...E };
                    !a?.data && E.isSearchResult ? (I.isSearchResultOnly = !0) : E.isSearchResult || (I.isSearchResultOnly = !1), a?.data?.labels && !E.labels ? ((I.labels = a.data.labels), (I.isPinnedOnly = !1)) : !a.data && E.labels && (I.isPinnedOnly = !0);
                    var p, v;
                    return !!E.conversation_id && !E.convo_label && delete I.convo_label, u && (I.max_entry_id = ((p = u), (v = a?.data?.max_entry_id), (0, _.KH)(p, v) ? p : v)), l && (I.min_entry_id = ((e, t) => (e && t ? ((0, _.T1)(e, t) ? t : e) : e || t))(l, a?.data?.min_entry_id)), { ...a, avatarMediaId: r, data: I, isUploading: i };
                },
                z = (e, t) => e[N.Yf].conversations[t],
                X = (e, t) => {
                    const s = z(e, t);
                    return s && s.fetchStatus ? (0, l.ke)(s.fetchStatus, s.data) : l.ZP.LOADED;
                },
                $ = (e, t) => {
                    const s = z(e, t);
                    return !(!s || !s.fetchStatus || s?.data?.isSearchResultOnly || s?.data?.isPinnedOnly) && (0, l.ke)(s.fetchStatus, s.data) === l.ZP.LOADED;
                },
                J = (e, t) => {
                    const s = z(e, t);
                    return !(!s || !s.fetchStatus) && s.fetchStatus[v.Yj.TOP] === l.ZP.LOADING;
                },
                ee = (e, t) => {
                    const s = z(e, t);
                    return !(!s || !s.fetchStatus) && s.fetchStatus[v.Yj.BOTTOM] === l.ZP.LOADING;
                },
                te = (0, n.P1)(
                    (e) => e[N.Yf].conversations,
                    (e) => Object.entries(e).reduce((e, [t, s]) => (s.data && !s.isDeleted && (e[t] = s), e), {}),
                ),
                se = (e) => Object.keys(te(e)).length > 0,
                ne = (e, t) => {
                    if (!t) return;
                    const s = $(e, t),
                        n = (0, O.g4)(e, N._2.SECONDARY),
                        a = (0, O.g4)(e, N._2.TERTIARY);
                    return s ? (n.includes(t) ? N._2.SECONDARY : a.includes(t) ? N._2.TERTIARY : N._2.PRIMARY) : void 0;
                },
                ae = (e, t) => {
                    const { data: s } = z(e, t) || {},
                        { sort_event_id: n } = s || {};
                    if (n) {
                        const t = p._h(e),
                            { by_user_id: s, id: a, message_data: r, sender_id: o } = (0, T.WK)(e, n) || {};
                        if ((o && o !== t) || (r && r.sender_id !== t) || (s && s !== t)) return a;
                    }
                    return null;
                },
                re = (e, t) => {
                    const s = ae(e, t),
                        { data: n } = z(e, t) || {};
                    return !s || !!(n && n.last_read_event_id && (0, _.KH)(n.last_read_event_id, s));
                },
                oe = (e, { conversationId: t }) => !!e[N.Yf]?.conversations[t]?.recipientCanNotDm,
                ie = (e, t) => {
                    const { avatarMediaId: s } = z(e, t) || {};
                    return s;
                },
                de = (e, t) => {
                    const { isUploading: s } = z(e, t) || {};
                    return s;
                },
                ce = (e, t) => I._T(e, ie(e, t)),
                _e =
                    (e, t) =>
                    (s, n, { api: a }) => {
                        const r = t.join(",");
                        return u
                            ._O(s, { params: { conversationId: e, joinedParticipantIds: r }, request: a.withEndpoint(i.Z).addParticipants })({ actionTypes: b, context: h, meta: { conversationId: e, participantIds: t } }, (e) => {
                                if (e) {
                                    const { entities: t } = e;
                                    return [t.entries && (0, T.I7)(t.entries), (0, E.dP)(t)];
                                }
                            })
                            .then((e) => {
                                const t = e.result.failed_participants;
                                if (!t) return Promise.resolve(e);
                                const s = Object.keys(t),
                                    n = 1 === s.length ? t[s[0]] : c.ZP.DefaultApiError,
                                    a = new d.Z("addParticpants URL", 500, e.headers, [{ code: n }]);
                                return (a.context.action = h), Promise.reject(a);
                            });
                    },
                ue =
                    (e = {}) =>
                    (t, s, { api: n }) => {
                        const { context: a, conversationId: r } = e;
                        return t(
                            (
                                (e) =>
                                (t, s, { api: n, featureSwitches: a }) => {
                                    if (e?.startsWith("e")) {
                                        const a = p._h(s()),
                                            r = e.replace(a, "").replace(/[e-]/g, "");
                                        return n
                                            .withEndpoint(i.Z)
                                            .fetchSecretPermissions({ recipient_ids: [r] })
                                            .then(({ permissions: { id_keys: s } }) => {
                                                if (!s[r].can_dm) return t({ payload: e, type: C });
                                            });
                                    }
                                    return Promise.resolve();
                                }
                            )(r),
                        ).then(() => {
                            if (!oe(s(), { conversationId: r })) {
                                const s = (e) => {
                                    if (e?.result?.conversation_timeline) {
                                        const { cards: t, entries: s, publishedArticles: n, tweets: r, users: o } = e.entities,
                                            { conversations: i, max_entry_id: d, min_entry_id: c, status: _ } = e.result.conversation_timeline,
                                            u = { max_entry_id: d, min_entry_id: c, status: _ },
                                            l = [f, g, D].includes(a)
                                                ? ((e = {}, t, s) => {
                                                      const n = t && s === f,
                                                          a = t && s === g;
                                                      return Object.entries(e).reduce((e, [s, r]) => {
                                                          let o;
                                                          return (o = n ? { ...r, max_search_entry_id: t?.max_entry_id, search_status: t?.status } : a ? { ...r, min_search_entry_id: t?.min_entry_id, status: t?.status } : { ...r, min_search_entry_id: t?.min_entry_id, max_search_entry_id: t?.max_entry_id, search_status: t?.status }), { ...e, [s]: o };
                                                      }, {});
                                                  })(i, u, a)
                                                : ((e = {}, t) => Object.entries(e).reduce((e, [s, n]) => ({ ...e, [s]: { ...n, ...t } }), {}))(i, u);
                                        return (0, O.s0)({ conversations: l, entries: s, users: o, tweets: r, cards: t, publishedArticles: n });
                                    }
                                };
                                return u._O(t, { params: e, request: n.withEndpoint(i.Z).fetchConversation })({ actionTypes: U, context: a, meta: { conversationId: r, direction: (0, v.Sj)(e) } }, s);
                            }
                        });
                    },
                le =
                    (e) =>
                    (t, s, { api: n }) =>
                        u._O(t, { params: { participantIds: e }, request: n.withEndpoint(i.Z).fetchConversationFromParticipants })({ actionTypes: w, meta: { participantIds: e } }),
                Ee =
                    (e = {}) =>
                    (t, s) => {
                        const n = s(),
                            { conversationId: a } = e,
                            r = ee(n, a),
                            o = $(n, a);
                        return r || o ? Promise.resolve() : t(ue({ conversationId: a, context: "FETCH_DM_CONVERSATION" }));
                    },
                Ie =
                    (e = {}) =>
                    (t, s) => {
                        const n = s(),
                            { conversationId: a } = e,
                            { data: r } = z(n, a) || {},
                            { max_search_entry_id: o } = r || {},
                            i = J(n, a),
                            d = f;
                        return o && !i ? t(ue({ conversationId: e.conversationId, min_id: o, context: d, min_position: o })) : Promise.resolve();
                    },
                pe =
                    (e = {}) =>
                    (t, s) => {
                        let n, a;
                        const r = s(),
                            { conversationId: o } = e,
                            { data: i } = z(r, o) || {},
                            { min_entry_id: d, min_search_entry_id: c, status: _ } = i || {};
                        e?.isSearch ? ((a = "FETCH_DM_CONVERSATION_RELATIVE_HISTORY"), (n = c)) : ((a = "FETCH_DM_CONVERSATION_HISTORY"), (n = d));
                        const u = ee(r, o);
                        return _ !== m.To.HAS_MORE || u ? Promise.resolve() : t(ue({ ...e, max_id: n, context: a }));
                    },
                ve =
                    (e = {}) =>
                    (t, s) => {
                        const n = s(),
                            { conversationId: a, entryId: r } = e;
                        return !J(n, a) && r ? t(ue({ conversationId: a, min_id: r, min_buffer: 15, context: D, min_position: r })) : Promise.resolve();
                    },
                Ae =
                    (e) =>
                    (t, s, { api: n }) =>
                        n
                            .withEndpoint(i.Z)
                            .fetchPermissions({ recipient_ids: e.join(",") })
                            .then(({ permissions: { id_keys: e } }) => e),
                Se =
                    (e) =>
                    (t, s, { api: n }) =>
                        u._O(t, { params: { conversationId: e }, request: n.withEndpoint(i.Z).acceptConversation })({ actionTypes: M, meta: { conversationId: e } }),
                me = (e, t) => Oe(e, t, !0),
                Te = (e, t) => Oe(e, t, !1),
                Oe =
                    (e, t, s) =>
                    (n, a, { api: r }) =>
                        u
                            ._O(n, { params: { userId: e, relationshipAction: s ? "mute" : "unmute", conversationId: t }, request: r.withEndpoint(i.Z).muteDMUser })({ actionTypes: P, meta: { userId: e, conversationId: t, mute: s } })
                            .then(n(S.ZP.patchUser(e, { dm_muting: s }))),
                ye =
                    (e = {}) =>
                    (t, s, { api: n }) => {
                        const { conversationId: a } = e;
                        return u.AB(t, { params: e, request: n.withEndpoint(i.Z).leaveConversation })({ actionTypes: L, context: "ACTION_LEAVE_CONVERSATION", meta: { conversationId: a } });
                    },
                Ne =
                    (e) =>
                    (t, s, { api: n }) => {
                        const a = e.join(",");
                        return u._O(t, { params: { conversationIds: a }, request: n.withEndpoint(i.Z).deleteConversations })({ actionTypes: Z, context: "LEAVE_CONVERSATIONS", meta: { conversationIds: e } }, (e) => {
                            if (!e.error) return [(0, A.fz)({ text: R })];
                        });
                    },
                Re = "rweb/directMessages/ADD_CONVERSATIONS",
                Ce = (e) => ({ payload: e, type: Re }),
                he = "rweb/directMessages/MARK_ALL_AS_READ",
                fe = (e) => ({ payload: e, type: he }),
                ge = (e, t) => ({ type: x.REQUEST, meta: { conversationId: e, lastReadEventId: t } }),
                De =
                    (e) =>
                    (t, s, { api: n }) => {
                        const a = s(),
                            { data: r = {} } = z(a, e) || {},
                            { sort_event_id: o } = r,
                            d = !re(a, e);
                        if (e && o && d) {
                            return u.AB(t, { params: { conversationId: e, last_read_event_id: o }, request: n.withEndpoint(i.Z).markRead })({ actionTypes: x, bypassFailureOnErrors: () => !0, context: "APP_DM_UPDATE_CONVERSATION_READ_STATE", meta: { conversationId: e, lastReadEventId: o } });
                        }
                        return Promise.resolve();
                    },
                Me = "rweb/directMessages/REMOVE_CONVERSATION",
                Pe = (e) => [{ payload: e, type: Me }, (0, T.J_)(e)],
                be = "rweb/directMessages/TOGGLE_CONVERSATION_NOTIFICATIONS",
                Ue = (e, t) => ({ meta: { conversationId: e }, payload: t, type: be }),
                we =
                    (e = {}) =>
                    (t, s, { api: n }) => {
                        const { conversationId: a, duration: r } = e;
                        return u.AB(t, { params: e, request: n.withEndpoint(i.Z).disableNotifications })({ actionTypes: V, context: "ACTION_DISABLE_NOTIFICATIONS", meta: { conversationId: a, duration: r } });
                    },
                Le =
                    (e = {}) =>
                    (t, s, { api: n }) => {
                        const { conversationId: a } = e;
                        return u.AB(t, { params: e, request: n.withEndpoint(i.Z).enableNotifications })({ actionTypes: Y, context: "ACTION_ENABLE_NOTIFICATIONS", meta: { conversationId: a } });
                    },
                Ze =
                    (e = {}) =>
                    (t, s, { api: n }) => {
                        const { conversationId: a } = e,
                            r = { mention_notifications_disabled: !0, ...e };
                        return u.AB(t, { params: r, request: n.withEndpoint(i.Z).updateMentionNotificationsSetting })({ actionTypes: B, context: "ACTION_DISABLE_DM_MENTION_NOTIFICATIONS", meta: { conversationId: a } });
                    },
                xe =
                    (e = {}) =>
                    (t, s, { api: n }) => {
                        const { conversationId: a } = e,
                            r = { mention_notifications_disabled: !1, ...e };
                        return u.AB(t, { params: r, request: n.withEndpoint(i.Z).updateMentionNotificationsSetting })({ actionTypes: F, context: "ACTION_ENABLE_DM_MENTION_NOTIFICATIONS", meta: { conversationId: a } });
                    },
                Be =
                    (e = {}) =>
                    (t, s, { api: n }) =>
                        n.withEndpoint(i.Z).updateTyping(e),
                Ve =
                    (e = {}) =>
                    (t, s, { api: n }) => {
                        const { conversationId: a, name: r } = e;
                        return u.AB(t, { params: e, request: n.withEndpoint(i.Z).updateConversationName })({ actionTypes: j, context: "ACTION_UPDATE_CONVERSATION_NAME", meta: { conversationId: a, name: r } });
                    },
                Fe = (e) => ({ type: j.SUCCESS, meta: { conversationId: e.conversation_id, name: e.conversation_name } }),
                Ye = "rweb/directMessages/UPDATE_CONVERSATION_MAX_ENTRY_ID",
                je = (e, t) => ({ meta: { conversationId: e }, payload: t, type: Ye }),
                He = "rweb/directMessages/ADD_AVATAR_MEDIA",
                qe = (e, t) => ({ meta: { conversationId: e }, payload: t, type: He }),
                ke = "rweb/directMessages/REMOVE_AVATAR_MEDIA",
                Qe = (e, t) => ({ meta: { conversationId: e }, payload: t, type: ke }),
                Ge = "rweb/directMessages/UPDATE_CONVERSATION_AVATAR",
                We = (e, t) => ({ meta: { conversationId: e }, payload: t, type: Ge }),
                Ke = "APP_DM_UPDATE_CONVERSATION_AVATAR",
                ze =
                    (e) =>
                    (t, s, { api: n }) => {
                        t({ meta: { conversationId: e }, type: H.REQUEST });
                        const a = ie(s(), e),
                            r = I.$i(a);
                        return t(r).then(
                            ([s]) => {
                                t({ meta: { conversationId: e }, type: H.SUCCESS });
                                return u._O(t, { params: { conversationId: e, avatar_id: s.uploadId }, request: n.withEndpoint(i.Z).updateConversationAvatar })({ actionTypes: q, context: Ke });
                            },
                            (s) => {
                                const n = { errors: s, context: Ke };
                                return t({ meta: { conversationId: e }, type: H.FAILURE }), Promise.reject(n);
                            },
                        );
                    },
                Xe =
                    (e, t) =>
                    (s, n, { api: a }) =>
                        u.Vg(s, { params: { conversation_id: e, welcome_message_id: t }, request: a.withEndpoint(i.Z).addWelcomeMessageToConversation })("ADD_WELCOME_MESSAGE_TO_CONVERSATION");
        },
        627279: (e, t, s) => {
            s.d(t, { I7: () => f, J_: () => P, MY: () => V, Ob: () => H, WK: () => k, WO: () => D, ZP: () => O, _d: () => W, gm: () => q, gz: () => Q, iN: () => G, kv: () => U, pt: () => b, px: () => L, rU: () => Y, s$: () => x, wC: () => g, zT: () => K });
            var n = s(468811),
                a = s.n(n),
                r = s(744531),
                o = s(933277),
                i = s(973710),
                d = s(166852),
                c = s(450681),
                _ = s(948322),
                u = s(856674),
                l = s(163889),
                E = s(917799),
                I = s(275365),
                p = s(849585),
                v = s(147143);
            const A = (0, E.dg)(v.A7, "DELETE_MESSAGE"),
                S = (0, E.dg)(v.A7, "MARK_ENTRIES_AS_SPAM"),
                m = (0, E.dg)(v.A7, "MARK_ENTRIES_AS_NOT_SPAM"),
                T = { entries: {}, entryIdsByConversationId: {}, reactionsByMessageId: {} };
            function O(e = T, t) {
                if (!t) return e;
                const { conversationId: s = t.payload?.conversation_id, requestId: n = t.payload?.id } = t.meta || {};
                switch (t.type) {
                    case w: {
                        const { id: n } = t.payload,
                            a = { ...e.entries, [n]: y(e.entries[n], { data: t.payload }) };
                        return { ...e, entries: a, entryIdsByConversationId: { ...e.entryIdsByConversationId, [s]: R([...(e.entryIdsByConversationId[s] || []), n], a) } };
                    }
                    case Z: {
                        const { id: n } = t.payload,
                            a = { ...e.entries, [n]: N(e.entries[n], t.payload) };
                        return { ...e, entries: a, entryIdsByConversationId: { ...e.entryIdsByConversationId, [s]: R([...(e.entryIdsByConversationId[s] || []), n], a) } };
                    }
                    case h: {
                        const s = Object.values(t.payload)
                                .filter((e) => e.type !== I.Cr.CONVERSATION_READ)
                                .reduce(
                                    (t, s) => {
                                        "message_edit" === s.type && ((s.id = s.message_data.id), (s.type = "message"));
                                        const { id: n } = s,
                                            a = (0, r.Z)(s, "message_reactions");
                                        return (
                                            (t.allEntries[n] = y(e.entries[n], { data: a, isLoading: !1, isLoaded: !0 })),
                                            s.message_reactions &&
                                                (s.message_reactions.forEach((n) => {
                                                    const a = { ...n, affects_sort: !0, message_id: s.id, type: I.Cr.REACTION_CREATE };
                                                    t.allEntries[a.id] = y(e.entries[a.id], { data: a, isLoading: !1, isLoaded: !0 });
                                                }),
                                                (t.embeddedReactionsByMessageId[n] = R([...(e.reactionsByMessageId[n] || []), ...s.message_reactions.map((e) => e.id)], { ...e.entries, ...t.allEntries }))),
                                            t
                                        );
                                    },
                                    { allEntries: {}, embeddedReactionsByMessageId: {} },
                                ),
                            n = { ...e.entries, ...s.allEntries },
                            a = (0, o.Z)(Object.values(s.allEntries), (e) => e.data.conversation_id),
                            i = Object.keys(a).reduce((t, s) => {
                                const r = a[s].map((e) => e.data.id);
                                return (t[s] = R([...(e.entryIdsByConversationId[s] || []), ...r], n)), t;
                            }, {});
                        return { ...e, entries: n, entryIdsByConversationId: { ...e.entryIdsByConversationId, ...i }, reactionsByMessageId: { ...e.reactionsByMessageId, ...s.embeddedReactionsByMessageId } };
                    }
                    case S.REQUEST:
                    case m.REQUEST: {
                        const s = t.type === S.REQUEST,
                            n = t.payload.reduce((t, n) => {
                                const a = e.entries[n] && e.entries[n].data;
                                return { ...t, [n]: { ...e.entries[n], data: { ...a, marked_as_spam: s } } };
                            }, {});
                        return { ...e, entries: { ...e.entries, ...n } };
                    }
                    case A.REQUEST:
                    case M: {
                        const n = t.type === M && !Array.isArray(t.payload),
                            a = e.entryIdsByConversationId[s] || [],
                            o = t.type === M ? (n ? a : t.payload) : [t.payload, ...(e.reactionsByMessageId[t.payload] || [])];
                        return { ...e, entries: (0, r.Z)(e.entries, o), entryIdsByConversationId: { ...(0, r.Z)(e.entryIdsByConversationId, s), ...(n ? {} : { [s]: (0, i.Z)(a, o) }) }, reactionsByMessageId: (0, r.Z)(e.reactionsByMessageId, o) };
                    }
                    case B: {
                        const a = t.payload.id || n,
                            o = { ...(0, r.Z)(e.entries, a) },
                            i = R(
                                (e.entryIdsByConversationId[s] || []).filter((e) => e !== a),
                                o,
                            ),
                            d = i ? { [s]: i } : {};
                        return { ...e, entries: o, entryIdsByConversationId: { ...(0, r.Z)(e.entryIdsByConversationId, s), ...d }, reactionsByMessageId: (0, r.Z)(e.reactionsByMessageId, a) };
                    }
                    case F: {
                        const s = t.payload,
                            n = [...(e.reactionsByMessageId[s.message_id] || []), s.id],
                            a = n.filter((t) => e.entries[t]);
                        n.length !== a.length && (0, l.ZP)("DM add reaction is missing a backing entry", { extra: { filteredMessageReactionIds: a, messageReactionIds: n, currentId: s.id } });
                        const r = R(a, e.entries);
                        return { ...e, reactionsByMessageId: { ...e.reactionsByMessageId, [s.message_id]: r } };
                    }
                    case j: {
                        const s = t.payload,
                            n = (e.reactionsByMessageId[s.message_id] || []).find((t) => e.entries[t].data.sender_id === s.sender_id);
                        if (!n) return e;
                        const a = (0, i.Z)(e.entryIdsByConversationId[s.conversation_id], n),
                            o = (0, i.Z)(e.reactionsByMessageId[s.message_id], n);
                        return { ...e, entries: (0, r.Z)(e.entries, n), entryIdsByConversationId: { ...e.entryIdsByConversationId, [s.conversation_id]: a }, reactionsByMessageId: { ...e.reactionsByMessageId, [s.message_id]: o } };
                    }
                    case p.t:
                        return T;
                    default:
                        return e;
                }
            }
            const y = (e, t) => {
                    const { error: s, isLoaded: n, isLoading: a, ...r } = e || {},
                        { data: o, ...i } = t || {};
                    return { ...r, data: { ...(r && r.data), ...o }, ...i };
                },
                N = (e, t) => {
                    const { data: s, ...n } = e || {};
                    return { ...n, data: { ...s, message_data: t } };
                },
                R = (e = [], t) => {
                    const s = (0, d.Z)(e).sort((e, s) => {
                        const n = (0, u.ZP)(t[e].data.time, t[s].data.time);
                        return 0 === n ? (0, u.ZP)(t[e].id, t[s].id) : n;
                    });
                    return s.length ? s : void 0;
                },
                C = (e, t, s, n) => ({ id: a().v1(), time: new Date().getTime().toString(), emoji_reaction: n, reaction_key: "Emoji", conversation_id: e, message_id: t, sender_id: s }),
                h = "rweb/directMessages/ADD_ENTRIES",
                f = (e) => ({ payload: e, type: h }),
                g =
                    (...e) =>
                    (t, s, { api: n }) => {
                        const a = C(...e);
                        return t([L(a.conversation_id, a.id, a), H(a), Y(a)]);
                    },
                D =
                    (...e) =>
                    (t, s, { api: n }) => {
                        const a = C(...e);
                        return t([H(a)]);
                    },
                M = "rweb/dm/REMOVE_CONVERSATION_ENTRIES",
                P = (e, t) => ({ meta: { conversationId: e }, payload: t, type: M }),
                b = (e, t) => ({ payload: e, type: t ? S.REQUEST : m.REQUEST }),
                U =
                    ({ conversationId: e, dmId: t, markSpam: s }) =>
                    (n, a, { api: r }) =>
                        (0, E.AB)(n, { params: { dm_id: t, report_as: s ? "spam" : "not_spam" }, request: r.withEndpoint(_.Z).reportSpam })({ actionTypes: s ? A : m, payload: s ? t : [t], meta: { conversationId: e } }),
                w = "rweb/directMessages/ADD_ENTRY",
                L = (e, t, s) => ({ meta: { conversationId: e, requestId: t }, payload: s, type: w }),
                Z = "rweb/directMessages/EDIT_ENTRY",
                x = (e, t, s) => ({ meta: { conversationId: e, requestId: t }, payload: s, type: Z }),
                B = "rweb/directMessages/REMOVE_ENTRY",
                V = (e, t, s) => ({ meta: { conversationId: e, requestId: t }, payload: s, type: B }),
                F = "rweb/directMessages/ADD_REACTION",
                Y = (e) => ({ payload: { ...e, type: I.Cr.REACTION_CREATE }, type: F }),
                j = "rweb/directMessages/DELETE_REACTION",
                H = ({ emoji_reaction: e, reaction_key: t, ...s }) => ({ payload: { ...s, type: I.Cr.REACTION_DELETE }, type: j }),
                q = (e) => e[v.Yf].entries,
                k = (e, t) => {
                    const s = q(e).entries[t];
                    return s && s.data;
                },
                Q = (e, t) => e[v.Yf].entries.entryIdsByConversationId[t],
                G = (e, t) => {
                    const s = Q(e, t);
                    return s ? (0, c.Z)(s.map((t) => k(e, t))) : [];
                },
                W = (e, t) => q(e).reactionsByMessageId[t],
                K = (e, t) => (W(e, t) || []).map((t) => k(e, t).sender_id);
        },
        720277: (e, t, s) => {
            s.d(t, { $7: () => se, A4: () => ne, B7: () => f, C2: () => W, Gk: () => M, Kf: () => x, MB: () => U, Oq: () => j, Pt: () => g, Re: () => Y, Sb: () => V, UD: () => P, Z3: () => B, ZI: () => H, ZP: () => O, aB: () => D, g4: () => Z, gN: () => R, j7: () => X, k5: () => F, kK: () => z, kL: () => ae, oH: () => G, s0: () => te, vC: () => $, xS: () => b });
            var n = s(161821),
                a = s(401388),
                r = s(948322),
                o = s(856674),
                i = s(234296),
                d = s(917799),
                c = s(312771),
                _ = s(56519),
                u = s(38562),
                l = s(275365),
                E = s(613195),
                I = s(627279),
                p = s(849585),
                v = s(923965),
                A = s(147143);
            const S = d.dg(A.A7, "FETCH_INBOX"),
                m = d.dg(A.A7, "UPDATE_LAST_SEEN_EVENT_ID"),
                T = { cursors: { [A.xR.Inbox]: { fetchStatus: c.ZP.NONE }, [A.xR.Trusted]: { fetchStatus: c.ZP.NONE }, [A.xR.Untrusted]: { fetchStatus: c.ZP.NONE }, [A.xR.UntrustedLowQuality]: { fetchStatus: c.ZP.NONE } }, lastSeenEventIds: {} };
            function O(e = T, t) {
                if (!t) return e;
                switch (t.type) {
                    case S.REQUEST: {
                        const { timelineType: s } = t.meta;
                        return { ...e, cursors: { ...e.cursors, [s]: { ...e.cursors[s], fetchStatus: c.ZP.LOADING } } };
                    }
                    case S.FAILURE: {
                        const { timelineType: s } = t.meta;
                        return { ...e, cursors: { ...e.cursors, [s]: { ...e.cursors[s], error: t.payload, fetchStatus: c.ZP.FAILED } } };
                    }
                    case S.SUCCESS: {
                        const { timelineType: s } = t.meta;
                        return { ...e, cursors: { ...e.cursors, [s]: { ...e.cursors[s], error: void 0, fetchStatus: c.ZP.LOADED } } };
                    }
                    case J:
                        return { ...e, cursors: { ...e.cursors, ...t.payload.cursors }, lastSeenEventIds: y(e.lastSeenEventIds, t.payload.lastSeenEventIds) };
                    case m.REQUEST:
                        return { ...e, lastSeenEventIds: { ...e.lastSeenEventIds, ...t.meta.lastSeenEventIds } };
                    case p.t:
                        return { ...T, cursors: { ...T.cursors, inbox: e.cursors.inbox } };
                    default:
                        return e;
                }
            }
            const y = (e, t) => ({ last_seen_event_id: N(e.last_seen_event_id, t.last_seen_event_id), trusted_last_seen_event_id: N(e.trusted_last_seen_event_id, t.trusted_last_seen_event_id), untrusted_last_seen_event_id: N(e.untrusted_last_seen_event_id, t.untrusted_last_seen_event_id) }),
                N = (e, t) => ((0, o.KH)(e, t) ? e : t),
                R = (e, t, s = "all") => {
                    const n = (0, u.Kp)(t),
                        a = (0, u.Rh)(t);
                    return n ? { ...e, filter_low_quality: a, include_quality: s } : e;
                },
                C = (e) => e[A.Yf].inbox,
                h = (e) => C(e).cursors,
                f = (e) => C(e).lastSeenEventIds,
                g = (e) => h(e)[A.xR.Inbox],
                D = (e) => h(e)[A.xR.Untrusted],
                M = (e) => h(e)[A.xR.UntrustedLowQuality],
                P = (e) => g(e).fetchStatus,
                b = (e) => P(e) === c.ZP.LOADING,
                U = (e) => P(e) === c.ZP.LOADED,
                w = { [A._2.PRIMARY]: (e) => e.trusted, [A._2.SECONDARY]: (e, t, s) => (t ? !(e.trusted || e.muted || (s && e.nsfw) || e.low_quality) : !(e.trusted || e.muted || (s && e.nsfw))), [A._2.TERTIARY]: (e, t, s) => (t ? !e.trusted && (e.muted || e.low_quality || (s && e.nsfw)) : !e.trusted && (e.muted || (s && e.nsfw))) },
                L = (e, t) => {
                    const s = (0, u.Rh)(e),
                        a = (0, u.a5)(e);
                    return (0, n.Z)((0, E.FS)(e)).filter((e) => w[t](e.data, s, a));
                },
                Z = (e, t) =>
                    L(e, t)
                        .map((e) => e.data)
                        .sort(i.Z)
                        .map((e) => e.conversation_id),
                x = (e) =>
                    L(e, A._2.PRIMARY)
                        .filter((e) => e?.data?.labels && e?.data?.labels?.find((e) => e.label_type === A.w1.PINNED))
                        .sort(i.l)
                        .map((e) => e?.data?.conversation_id),
                B = (e, { tag: t }) =>
                    L(e, A._2.PRIMARY)
                        .filter((e) => e?.data?.tag?.value && e.data.tag.value === t?.value)
                        .map((e) => e?.data?.conversation_id),
                V = (e) => {
                    const t = L(e, A._2.PRIMARY),
                        s = L(e, A._2.SECONDARY),
                        n = t.concat(s),
                        { last_seen_event_id: a } = f(e);
                    return q(e, n, a);
                },
                F = (e) => {
                    const t = L(e, A._2.SECONDARY),
                        { untrusted_last_seen_event_id: s } = f(e);
                    return q(e, t, s);
                },
                Y = (e) => {
                    let t = 0,
                        s = 0,
                        n = 0,
                        a = 0;
                    const r = L(e, A._2.SECONDARY),
                        { untrusted_last_seen_event_id: o } = f(e);
                    return (
                        r.forEach((r) => {
                            const i = r.data?.conversation_id,
                                d = o && k(e, i, o),
                                c = Q(r?.data?.social_proof);
                            d ? (c ? (t += 1) : (n += 1)) : c ? (s += 1) : (a += 1);
                        }),
                        { newUntrustedWithSocialProof: t, oldUntrustedWithSocialProof: s, newUntrustedWithoutSocialProof: n, oldUntrustedWithoutSocialProof: a }
                    );
                },
                j = (e) => {
                    const t = L(e, A._2.SECONDARY),
                        s = L(e, A._2.TERTIARY);
                    return t.length + s.length;
                },
                H = (e) => L(e, A._2.TERTIARY).length,
                q = (e, t, s) => (s ? t.reduce((t, { data: { conversation_id: n } }) => (k(e, n, s) ? t + 1 : t), 0) : 0),
                k = (e, t, s) => {
                    const n = (0, E.gq)(e, t),
                        a = (0, E.jT)(e, t);
                    return !!n && (0, o.T1)(n, s) && !a;
                },
                Q = (e) => {
                    if (!e || 0 === e.length) return !1;
                    const t = e[0];
                    return t.proof_type === l.UN.MUTUAL_FRIENDS && 0 !== t.total;
                },
                G =
                    (e = {}) =>
                    (t, s, { api: n, featureSwitches: a }) => {
                        const o = s(),
                            i = (0, u.a5)(o),
                            c = R({ nsfw_filtering_enabled: i, ...e }, s());
                        return d._O(t, { params: c, request: n.withEndpoint(r.Z).fetchUserInbox })({ actionTypes: S, context: "FETCH_DM_INBOX", meta: { timelineType: A.xR.Inbox } }, (e) => {
                            if (e) {
                                const { cards: t, entries: s, publishedArticles: n, tweets: a, users: r } = e.entities;
                                return te({ ...e.result.inbox_initial_state, entries: s, users: r, tweets: a, cards: t, publishedArticles: n });
                            }
                        });
                    },
                W =
                    (e = {}) =>
                    (t, s) => {
                        const n = s(),
                            a = P(n);
                        return a !== c.ZP.LOADED && a !== c.ZP.LOADING ? t(G(e)) : Promise.resolve();
                    },
                K =
                    (e, t, s = 0) =>
                    (n, a, { api: o }) => {
                        const i = a(),
                            c = (0, u.a5)(i),
                            { min_entry_id: _, status: E } = h(i)[e],
                            I = e === A.xR.UntrustedLowQuality ? A.xR.Untrusted : e,
                            p = { ...t, max_id: _, timelineType: I, nsfw_filtering_enabled: c };
                        if (E !== l.To.HAS_MORE) return Promise.resolve();
                        return d
                            ._O(n, { params: p, request: o.withEndpoint(r.Z).fetchInboxHistory })({ actionTypes: S, context: "FETCH_DM_INBOX_HISTORY", meta: { timelineType: e } }, (t) => {
                                if (t) {
                                    const { cards: s, entries: n, publishedArticles: a, tweets: r, users: o } = t.entities,
                                        { inbox_timeline: i } = t.result;
                                    return te({ conversations: i.conversations, inbox_timelines: { [e]: i }, entries: n, users: o, tweets: r, cards: s, publishedArticles: a });
                                }
                            })
                            .then((a) => {
                                const r = a.result.inbox_timeline;
                                if (s < 3 && !r.conversations && r.status === l.To.HAS_MORE) return n(K(e, t, s + 1));
                            });
                    },
                z = (e) => (t, s) => {
                    const n = R(e, s(), "high");
                    return t(K(A.xR.Untrusted, n));
                },
                X = (e) => (t, s) => {
                    const n = R(e, s(), "low");
                    return t(K(A.xR.UntrustedLowQuality, n));
                },
                $ = (e) => (t, s) => {
                    const n = R(e, s());
                    return t(K(A.xR.Trusted, n));
                },
                J = "rweb/directMessages/UPDATE_INBOX",
                ee = (e) => {
                    const { inbox_timelines: t, ...s } = e;
                    return { payload: { cursors: t, lastSeenEventIds: s }, type: J };
                },
                te = (e = {}) => {
                    const { cards: t, conversations: s, cursor: n, entries: r, publishedArticles: o, tweets: i, users: d, ...c } = e;
                    return [(0, _.dP)({ cards: t, tweets: i, users: d, publishedArticles: o }), !(0, a.Z)(s) && (0, E.tw)(s), !(0, a.Z)(r) && (0, I.I7)(r), !(0, a.Z)(n) && (0, v.NJ)(n), !(0, a.Z)(c) && ee(c)].filter((e) => e);
                },
                se = () => (e) => e(ae({ withTrusted: !0 })),
                ne = () => (e) => e(ae({ withUntrusted: !0 })),
                ae =
                    ({ withTrusted: e, withUntrusted: t } = {}) =>
                    (s, n, { api: a }) => {
                        const i = n(),
                            c = (0, E.FS)(i),
                            _ = f(i),
                            { last_seen_event_id: u, trusted_last_seen_event_id: l, untrusted_last_seen_event_id: I } = _,
                            p = Object.values(c).reduce((e, { data: { sort_event_id: t, trusted: s } }) => ((e.last_seen_event_id = N(e.last_seen_event_id, t)), (e.trusted_last_seen_event_id = s ? N(e.trusted_last_seen_event_id, t) : e.trusted_last_seen_event_id), (e.untrusted_last_seen_event_id = s ? e.untrusted_last_seen_event_id : N(e.untrusted_last_seen_event_id, t)), e), { ..._ });
                        if ((0, o.T1)(p.last_seen_event_id, u) || (e && (0, o.T1)(p.trusted_last_seen_event_id, l)) || (t && (0, o.T1)(p.untrusted_last_seen_event_id, I))) {
                            e || delete p.trusted_last_seen_event_id, t || delete p.untrusted_last_seen_event_id;
                            return d.AB(s, { params: p, request: a.withEndpoint(r.Z).updateLastSeenEventId })({ actionTypes: m, context: "APP_DM_UPDATE_LAST_SEEN", meta: { lastSeenEventIds: p } });
                        }
                        return Promise.resolve();
                    };
        },
        296196: (e, t, s) => {
            s.r(t),
                s.d(t, {
                    GLOBAL_STATE_KEY: () => p.Yf,
                    acceptConversation: () => r.tF,
                    addAvatarMedia: () => r._D,
                    addParticipants: () => r.ck,
                    addRecentSearches: () => M,
                    addWelcomeMessageToConversation: () => r.Qe,
                    clearRecentSearch: () => P,
                    clearRecentSearches: () => b,
                    clearSearchCache: () => O,
                    default: () => L,
                    deleteConversations: () => r.S9,
                    disableMentionNotifications: () => r.BW,
                    disableNotifications: () => r.dq,
                    enableMentionNotifications: () => r.Ap,
                    enableNotifications: () => r.PZ,
                    fetchConversation: () => r.Um,
                    fetchConversationFromParticipants: () => r.sS,
                    fetchConversationFuture: () => r.OW,
                    fetchConversationHistoricalSnapshot: () => r.wm,
                    fetchConversationHistory: () => r.Be,
                    fetchConversationIfNeeded: () => r.sA,
                    fetchInbox: () => i.oH,
                    fetchInboxIfNeeded: () => i.C2,
                    fetchPermissions: () => r.qg,
                    fetchTrustedInboxHistory: () => i.vC,
                    fetchUntrustedInboxHistory: () => i.kK,
                    fetchUntrustedLowQualityInboxHistory: () => i.j7,
                    fetchUpdates: () => U.f9,
                    fetchUpdatesIfNeeded: () => U.hd,
                    leaveConversation: () => r.qH,
                    markAllAsRead: () => r.SF,
                    markConversationRead: () => r.v3,
                    markSpamStatus: () => o.kv,
                    muteDMUser: () => r.sj,
                    removeAvatarMedia: () => r.Xw,
                    removeConversation: () => r.Ae,
                    removeEntry: () => o.MY,
                    searchConversations: () => D,
                    selectConversation: () => r.xu,
                    selectConversationAvatarMediaId: () => r.aG,
                    selectConversationAvatarMediaIsUploading: () => r.W,
                    selectConversationAvatarMediaUploadProgress: () => r.UA,
                    selectConversationFetchStatus: () => r.Pr,
                    selectConversationIdsByInbox: () => i.g4,
                    selectConversationIsLoaded: () => r.gQ,
                    selectConversationIsRead: () => r.jT,
                    selectConversations: () => r.FS,
                    selectEntries: () => o.gm,
                    selectEntriesForConversation: () => o.iN,
                    selectEntry: () => o.WK,
                    selectFetchStatus: () => i.UD,
                    selectHasConversations: () => r.ax,
                    selectInboxCursor: () => i.Pt,
                    selectInboxIsLoaded: () => i.MB,
                    selectInboxTypeByConversation: () => r.Zg,
                    selectIsLoadingTop: () => i.xS,
                    selectLabelledConversationIds: () => i.Z3,
                    selectLastSeenEventIds: () => i.B7,
                    selectPinnedConversationIds: () => i.Kf,
                    selectRecentSearches: () => h,
                    selectRecipientCanNotDm: () => r.OF,
                    selectSearchConversations: () => g,
                    selectSearchFetchStatus: () => f,
                    selectUnreadCount: () => i.Sb,
                    selectUntrustedConversationCount: () => i.Oq,
                    selectUntrustedCounts: () => i.Re,
                    selectUntrustedCursor: () => i.aB,
                    selectUntrustedLowQualityConversationCount: () => i.ZI,
                    selectUntrustedLowQualityCursor: () => i.Gk,
                    selectUntrustedUnreadCount: () => i.k5,
                    selectUpdates: () => U.o8,
                    setConversationAvatar: () => r.X5,
                    toggleNotifications: () => r.$S,
                    unmuteDMUser: () => r.GP,
                    updateConversationAvatar: () => r.nI,
                    updateConversationName: () => r.FI,
                    updateConversationReadState: () => r.d0,
                    updateLastSeenEventId: () => i.kL,
                    updateTrustedLastSeenEventId: () => i.$7,
                    updateTyping: () => r.lT,
                    updateUntrustedLastSeenEventId: () => i.A4,
                });
            var n = s(118717),
                a = s(499627),
                r = s(613195),
                o = s(627279),
                i = s(720277),
                d = (s(136728), s(468811)),
                c = s.n(d),
                _ = s(948322),
                u = s(917799),
                l = s(312771),
                E = s(390387),
                I = s(849585),
                p = s(147143);
            const v = "rweb.recentDMSearches",
                A = Object.freeze({ REQUEST: "rweb/directMessages/SEARCH_REQUEST", SUCCESS: "rweb/directMessages/SEARCH_SUCCESS", FAILURE: "rweb/directMessages/SEARCH_FAILURE" }),
                S = "rweb/directMessages/ADD_RECENT_SEARCHES",
                m = (e) => ({ payload: e, type: S }),
                T = "rweb/directMessages/CLEAR_SEARCH_CACHE",
                O = () => ({ type: T }),
                y = "rweb/directMessages/CLEAR_RECENT_SEARCH",
                N = "rweb/directMessages/CLEAR_RECENT_SEARCHES",
                R = { searchResults: {}, recentSearches: [] };
            function C(e = R, t) {
                if (!t) return e;
                switch (t.type) {
                    case S:
                    case y:
                        return { ...e, recentSearches: t.payload || [] };
                    case N:
                        return { ...e, recentSearches: [] };
                    case A.REQUEST: {
                        const { fetchNextPage: s, preserveResults: n, query: a } = t.meta || {},
                            r = e.searchResults ? e.searchResults[a] : {},
                            o = { ...e.searchResults, [a]: { ...r, fetchStatus: n || s ? r?.fetchStatus : l.ZP.LOADING } };
                        return { ...e, searchResults: o };
                    }
                    case A.FAILURE: {
                        const { fetchNextPage: s, preserveResults: n, query: a } = t.meta || {},
                            r = e.searchResults ? e.searchResults[a] : {},
                            o = { ...e.searchResults, [a]: { ...r, fetchStatus: n || s ? r?.fetchStatus : l.ZP.FAILED, error: t.payload } };
                        return { ...e, searchResults: o };
                    }
                    case A.SUCCESS: {
                        const { query: s } = t.meta || {},
                            n = t.payload && t.payload.result ? t.payload.result : null,
                            { conversations: a, error: r } = n || {},
                            o = n?.conversationIds ? n.conversationIds : a ? Object.keys(a) : [],
                            i = e.searchResults ? e.searchResults[s] : {},
                            d = i?.conversationIds || [],
                            c = d.length && o ? d.concat(o.filter((e) => !d.includes(e))) : o,
                            _ = { ...e.searchResults, [s]: { ...i, lastCursor: n?.cursor, conversationIds: c, error: r, fetchStatus: l.ZP.LOADED } };
                        return { ...e, searchResults: _ };
                    }
                    case T:
                    case I.t:
                        return { ...R, recentSearches: e.recentSearches };
                    default:
                        return e;
                }
            }
            const h = (e) => e[p.Yf].search?.recentSearches,
                f = (e, t) => {
                    const s = e[p.Yf].search?.searchResults;
                    if (s) {
                        const e = s[t];
                        return e?.fetchStatus || l.ZP.NONE;
                    }
                    return l.ZP.NONE;
                },
                g = (e, t) => {
                    const s = e[p.Yf].search?.searchResults;
                    return (s && s[t]?.conversationIds) || [];
                },
                D =
                    (e) =>
                    (t, s, { api: n }) => {
                        const a = s(),
                            { fetchNextPage: r, preserveResults: o, query: d } = e || {},
                            l = { query: d, search_type: "conversations", request_id: c().v4() },
                            E = ((e, t) => {
                                const s = e[p.Yf].search?.searchResults;
                                return (s && s[t]?.lastCursor) || null;
                            })(a, d),
                            I = ((e) => e[p.Yf].search?.searchResults)(a),
                            v = I ? I[d] : void 0,
                            S = !!E;
                        E && d && r && (l.cursor = E);
                        const m = u._O(t, { params: l, request: n.withEndpoint(_.Z).search }),
                            T = (e) => {
                                if (e && e.result) {
                                    const { conversations: t } = e.result,
                                        { users: s } = e.entities;
                                    return t && Object.values(t).forEach((e) => (e.isSearchResult = !0)), t ? (0, i.s0)({ conversations: t, users: s }) : null;
                                }
                            };
                        return !v || v?.error || o || (r && S) ? m({ actionTypes: A, context: "SEARCH", meta: { query: l.query, cursor: l.cursor, preserveResults: o, fetchNextPage: r } }, T) : (t({ type: "rweb/directMessages/SEARCH_SUCCESS", payload: { result: v }, meta: { query: d, cursor: E } }), Promise.resolve());
                    },
                M =
                    (e) =>
                    (t, s, { featureSwitches: n, userPersistence: a }) => {
                        const r = n.getNumberValue("dm_inbox_search_max_recent_searches_stored");
                        return a.get(v).then((s) => {
                            const n = s?.recentSearches || [];
                            return !n.includes(e) && n.push(e), n.length > r && n.shift(), t(m(n)), a.set(v, { recentSearches: n });
                        });
                    },
                P =
                    (e) =>
                    (t, s, { userPersistence: n }) =>
                        n.get(v).then((s) => {
                            const a = s?.recentSearches || [];
                            return a.splice(a.indexOf(e), 1), t({ payload: a, type: y }), n.set(v, { recentSearches: a });
                        }),
                b =
                    () =>
                    (e, t, { userPersistence: s }) => (e({ type: N }), s.set(v, { recentSearches: [] }));
            a.Z.register(
                { [p.Yf]: C },
                () =>
                    (e, t, { userPersistence: s }) =>
                        E.Qb(t())
                            ? Promise.resolve(
                                  s.get(v).then((t) => {
                                      const s = t?.recentSearches;
                                      s && e(m(s));
                                  }),
                              ).then(() => {})
                            : Promise.resolve(),
            );
            var U = s(923965);
            const w = (0, n.UY)({ conversations: r.ZP, entries: o.ZP, inbox: i.ZP, search: C, updates: U.ZP });
            a.Z.register({ [p.Yf]: w });
            const L = w;
        },
        849585: (e, t, s) => {
            s.d(t, { m: () => a, t: () => n });
            const n = "rweb/directMessages/RESET",
                a = () => ({ type: n });
        },
    },
]);
//# sourceMappingURL=https://ton.local.twitter.com/responsive-web-internal/sourcemaps/client-web/bundle.Report-8ed0fac5.9624faea.js.map
