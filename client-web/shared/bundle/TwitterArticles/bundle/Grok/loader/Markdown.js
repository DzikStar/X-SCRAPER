        ...
        719870: (e, t, n) => {
            ...
            (y.def = f(y.def).replace("label", y._label).replace("title", y._title).getRegex()),
                ...
                (y.html = f(y.html, "i")
                    ...
                    .getRegex()),
                (y.lheading = f(y.lheading).replace(/bull/g, y.bullet).getRegex()),
                (y.paragraph = f(y._paragraph).replace("hr", y.hr).replace("heading", " {0,3}#{1,6}(?:\\s|$)").replace("|lheading", "").replace("|table", "").replace("blockquote", " {0,3}>").replace("fences", " {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list", " {0,3}(?:[*+-]|1[.)]) ").replace("html", "</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag", y._tag).getRegex()),
            ...
            ($.punctuation = f($.punctuation, "u")
                .replace(/punctuation/g, $._punctuation)
                .getRegex()),
                ($.blockSkip = /\[[^[\]]*?\]\([^\(\)]*?\)|`[^`]*?`|<[^<>]*?>/g),
                ($.anyPunctuation = /\\[punct]/g),
                ($._escapes = /\\([punct])/g),
                ($._comment = f(y._comment).replace("(?:--\x3e|$)", "--\x3e").getRegex()),
                ($.emStrong.lDelim = f($.emStrong.lDelim, "u").replace(/punct/g, $._punctuation).getRegex()),
                ($.emStrong.rDelimAst = f($.emStrong.rDelimAst, "gu").replace(/punct/g, $._punctuation).getRegex()),
                ($.emStrong.rDelimUnd = f($.emStrong.rDelimUnd, "gu").replace(/punct/g, $._punctuation).getRegex()),
                ($.anyPunctuation = f($.anyPunctuation, "gu").replace(/punct/g, $._punctuation).getRegex()),
                ($._escapes = f($._escapes, "gu").replace(/punct/g, $._punctuation).getRegex()),
                ($._scheme = /[a-zA-Z][a-zA-Z0-9+.-]{1,31}/),
                ($._email = /[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+(@)[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)+(?![-_])/),
                ($.autolink = f($.autolink).replace("scheme", $._scheme).replace("email", $._email).getRegex()),
                ($._attribute = /\s+[a-zA-Z:_][\w.:-]*(?:\s*=\s*"[^"]*"|\s*=\s*'[^']*'|\s*=\s*[^\s"'=<>`]+)?/),
                ($.tag = f($.tag).replace("comment", $._comment).replace("attribute", $._attribute).getRegex()),
                ($._label = /(?:\[(?:\\.|[^\[\]\\])*\]|\\.|`[^`]*`|[^\[\]\\`])*?/),
                ($._href = /<(?:\\.|[^\n<>\\])+>|[^\s\x00-\x1f]*/),
                ($._title = /"(?:\\"?|[^"\\])*"|'(?:\\'?|[^'\\])*'|\((?:\\\)?|[^)\\])*\)/),
                ($.link = f($.link).replace("label", $._label).replace("href", $._href).replace("title", $._title).getRegex()),
                ($.reflink = f($.reflink).replace("label", $._label).replace("ref", y._label).getRegex()),
                ($.nolink = f($.nolink).replace("ref", y._label).getRegex()),
                ($.reflinkSearch = f($.reflinkSearch, "g").replace("reflink", $.reflink).replace("nolink", $.nolink).getRegex()),
                ($.normal = { ...$ }),
                ($.pedantic = {
                    ...$.normal,
                ...
                ($.gfm = { ...$.normal, escape: f($.escape).replace("])", "~|])").getRegex(), _extended_email: /[A-Za-z0-9._+-]+(@)[a-zA-Z0-9-_]+(?:\.[a-zA-Z0-9-_]*[a-zA-Z0-9])+(?![-_])/, url: /^((?:ftp|https?):\/\/|www\.)(?:[a-zA-Z0-9\-]+\.?)+[^\s<]*|^email/, _backpedal: /(?:[^?!.,:;*_'"~()&]+|\([^)]*\)|&(?![a-zA-Z0-9]+;$)|[?!.,:;*_'"~)]+(?!$))+/, del: /^(~~?)(?=[^\s~])([\s\S]*?[^\s~])\1(?=[^~]|$)/, text: /^([`~]+|[^`~])(?:(?= {2,}\n)|(?=[a-zA-Z0-9.!#$%&'*+\/=?_`{\|}~-]+@)|[\s\S]*?(?:(?=[\\<!\[`*~_]|\b_|https?:\/\/|ftp:\/\/|www\.|$)|[^ ](?= {2,}\n)|[^a-zA-Z0-9.!#$%&'*+\/=?_`{\|}~-](?=[a-zA-Z0-9.!#$%&'*+\/=?_`{\|}~-]+@)))/ }),
                ($.gfm.url = f($.gfm.url, "i").replace("email", $.gfm._extended_email).getRegex()),
                ($.breaks = {
                    ...$.gfm,
            ...
            class T {
                ...
                textRenderer;
                constructor(e) {
                    (this.options = e || r), (this.options.renderer = this.options.renderer || new S()), (this.renderer = this.options.renderer), (this.renderer.options = this.options), (this.textRenderer = new R());
                }
                static parse(e, t) {
            ...
            (E.options = E.setOptions =
                ...
                (E.Hooks = A),
                (E.parse = E);
            E.options, E.setOptions, E.use, E.walkTokens, E.parseInline, T.parse, z.lex;
        },
    },
]);
